...
treeSize :: Tree a -> Int
treeSize Empty = 0
treeSize (Node a s t) = 1 + (treeSize s) + (treeSize t)
...
treeMax Empty = 0
treeMax (Node a s t) = max a $ max (treeMax s) (treeMax t)
...
allValues condition (Node a s t) = (condition a) && (allValues condition s) && (allValues condition t)
...
mapTree f (Node a s t) = Node (f a) (mapTree f s) (mapTree f t)
...
cull val (Node a s t) = if val == a
                        then Empty
                        else Node a (cull val s) (cull val t)
...
isOrdered (Node a s t) = (allValues (<a) s) && ( allValues (>a) t) && (isOrdered s) && (isOrdered t)
...
walk [] (Node a s t) = Just a
walk (StepL:steps) (Node a s t) = walk steps s
walk (StepR:steps) (Node a s t) = walk steps t
...
set [] val (Node a s t) = Node val s t
set (StepL:steps) val (Node a s t) = Node a (set steps val s) t
set (StepR:steps) val (Node a s t) = Node a s (set steps val t)
...
search val (Node a s t) 
  | val == a = Just []
  | otherwise = case search val s of 
                  Just steps -> Just (StepL:steps)
                  Nothing -> case search val t of 
