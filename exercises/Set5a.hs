...
totalPrice (MkShoppingEntry s p c) = (fromIntegral c) * p
...
buyOneMore (MkShoppingEntry s p c) = MkShoppingEntry s p (c+1)
...
up p = MkPosition (getX p) (1 + (getY p))

-- right increases the x value of a position by one
right :: Position -> Position
right p = MkPosition (1 + (getX p)) (getY p)
...
tick :: UpDown -> UpDown
tick (Incr k) = Incr (k+1)
tick (Decr k) = Decr (k-1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (Incr k) = Decr k
toggle (Decr k) = Incr k
...
rgb Green = [0,1,0]
rgb Blue = [0,0,1]
rgb (Mix a b) = map (\x -> x/2) $ zipWith (+) (rgb a) (rgb b)
...
toList Empty = []
toList (Pair k v keys) = (k,v) : toList keys
...
toBin 0 = O End
toBin 1 = I End
toBin k = if (mod k 2) == 0 
          then (O (toBin (div k 2)))
          else (I (toBin (div k 2)))
