...
longest :: (Ord a) => [[a]] -> [a]
longest xss = minimum $ filter ((== maxLen) . length) xss
...
incrementKey k kvs = map (\(x, y) -> if (x == k) then (x, y+1) else (x, y)) kvs 
...
freqs [] = Map.empty
freqs xs = foldr (\x m -> Map.alter incr x m) Map.empty xs
    where incr k = case k of 
                    Nothing -> Just 1
                    Just n -> Just (n+1)
...
maxIndex arr = foldr max' (head $ indices arr) (indices arr) 
    where max' i j = if (arr ! i >= arr ! j) then i else j