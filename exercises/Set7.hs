...
data State = Start | Error | Finished | Eggs | Flour | Sugar | Mixed | SugarFlour
  deriving (Eq,Show)
...
passwordAllowed s (ContainsSome t) = not $ null [c | c <- s, (elem c t)] 
passwordAllowed s (DoesNotContain t) = not $ passwordAllowed s (ContainsSome t)
passwordAllowed s (And a b) = (passwordAllowed s a) && (passwordAllowed s b)
passwordAllowed s (Or a b) = (passwordAllowed s a) || (passwordAllowed s b)
