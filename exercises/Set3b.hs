...
sums' j k i = if (j == i + 1) then [] else k : sums' (j+1) (k + j+1) i
...
indexDefault (first:_) 0 def = first
indexDefault (first:rest) k def = indexDefault rest (k-1) def
...
mymaximum bigger initial (x:rest) = if (bigger initial x) then (mymaximum bigger initial rest) else (mymaximum bigger x rest)
